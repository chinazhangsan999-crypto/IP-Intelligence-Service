...
import { extractMmdbFromTarGz } from './lib/tarArchive.js';
  ...
  return new Promise((resolve, reject) => {
    const request = https.get(url, { headers: { 'user-agent': agent, accept: '*/*', ...headers } }, (response) => {
      if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
        ...
        const nextHeaders = nextUrl.origin === new URL(url).origin
          ? headers
          : Object.fromEntries(Object.entries(headers).filter(([name]) => name.toLowerCase() !== 'authorization'));
        return resolve(get(nextUrl, nextHeaders, redirects - 1));
      }
      if (response.statusCode !== 200) {
        response.resume();
        const error = new Error(`Download failed with HTTP ${response.statusCode}`);
...
async function download(url, headers = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try { return await get(url, headers); } catch (error) { failure = error; if (attempt < 2) await new Promise((resolve) => setTimeout(resolve, 800 * (attempt + 1))); }
  }
  throw failure;
}
...
async function geolite() { const account = String(process.env.MAXMIND_ACCOUNT_ID || '').trim(); const key = String(process.env.MAXMIND_LICENSE_KEY || '').trim(); if (!account || !key) throw new Error('MaxMind 凭据尚未配置'); const basic = `Basic ${Buffer.from(`${account}:${key}`).toString('base64')}`; const definitions = [['Country', 'maxmind-geolite2-country.mmdb'], ['City', 'maxmind-geolite2-city.mmdb'], ['ASN', 'maxmind-geolite2-asn.mmdb']]; const output = []; for (const [edition, file] of definitions) { let archive; try { archive = await get(`https://download.maxmind.com/geoip/databases/GeoLite2-${edition}/download?suffix=tar.gz`, { authorization: basic }); } catch (error) { if (error.statusCode === 401) throw new Error('MaxMind Account ID 或 License Key 无效'); if (error.statusCode === 403 || error.statusCode === 404) throw new Error(`MaxMind 账户尚未获得 GeoLite2 ${edition} 下载权限，请在 MaxMind 后台接受 GeoLite2 EULA 并启用该数据库`); if (error.statusCode === 429) throw new Error('MaxMind 已达到当日下载次数限制，请稍后重试'); throw error; } const mmdb = extractMmdbFromTarGz(archive, maxBytes); const temp = path.join(config.dataDir, `.${file}-${randomUUID()}`); await fs.writeFile(temp, mmdb, { mode: 0o600 }); try { const reader = await maxmind.open(temp); if (!reader.get('8.8.8.8')) throw new Error(`GeoLite2 ${edition} 数据校验失败`); await atomic(path.join(config.dataDir, file), mmdb); output.push(report(`maxmind-geolite2-${edition.toLowerCase()}`, file, mmdb, 1, reader.metadata?.buildEpoch?.toISOString?.().slice(0, 10))); } finally { await fs.rm(temp, { force: true }); } } return output; }
async function spamhaus() { const output = []; for (const [id, url, file] of [['spamhaus-drop-v4', 'https://www.spamhaus.org/drop/drop_v4.json', 'spamhaus-drop-v4.json'], ['spamhaus-drop-v6', 'https://www.spamhaus.org/drop/drop_v6.json', 'spamhaus-drop-v6.json']]) { const raw = await download(url); const text = raw.toString('utf8').trim(); const payload = text.startsWith('[') || text.startsWith('{"drop"') ? JSON.parse(text) : text.split(/\r?\n/).filter(Boolean).map((line) => JSON.parse(line)); const records = (payload.drop || payload).filter?.((x) => validCidr(x?.cidr || x?.prefix || x)) || []; if (records.length < 1) throw new Error(`${id} has no valid CIDRs`); const out = Buffer.from(JSON.stringify({ attribution: 'Spamhaus Project © Spamhaus', source_date: new Date().toISOString(), records }) + '\n'); await atomic(path.join(config.dataDir, file), out); output.push(report(id, file, out, records.length)); } return output; }
...
try {
  ...
  process.stdout.write(`${JSON.stringify({ status: 'failed', updated: false, source_id: source, error: String(error.message || error).slice(0, 500), sources: [] })}\n`);
}

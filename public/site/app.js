(() => {
  ...
  function renderSummary(data) {
    const location = [countryName(data.country_code, data.country_name), data.region, data.city].filter((value, index, list) => value && value !== t('unknown') && list.indexOf(value) === index).join(language === 'zh' ? ' · ' : ', ') || t('unknown');
  ...
  function renderEvidence(evidence) {
    ...
    for (const row of rows.slice(0, 60)) {
      ...
      strong.textContent = labels[language].fields[row.field] || text(row.field); small.textContent = t('evidenceMeta', { source: text(row.source, t('unknownSource')), confidence: translated('confidence', row.confidence) }); code.textContent = text(row.value);
  ...
  function sourceName(source) { const names = { 'cloud-ranges': ['云厂商官方网段', 'Official cloud ranges'], 'tor-exit': ['Tor 官方出口列表', 'Official Tor exit list'], 'apple-private-relay': ['Apple 隐私中继网段', 'Apple Private Relay ranges'], 'special-purpose': ['IANA 特殊用途地址', 'IANA special-purpose registry'], 'ripe-ris': ['RIPE RIS', 'RIPE RIS'], 'caida-as2org': ['CAIDA 组织登记', 'CAIDA AS-to-Organization'], peeringdb: ['PeeringDB', 'PeeringDB'] }; return names[source]?.[language === 'zh' ? 0 : 1] || source; }
  ...
  function renderResult(data, meta) {
    ...
    elements['location-country'].textContent = countryName(data.country_code, data.country_name); elements['location-detail'].textContent = [data.region, data.city].filter((value, index, list) => value && list.indexOf(value) === index).join(' · ') || t('noFineLocation');
    ...
    elements['isp-value'].textContent = text(data.isp); elements['timezone-value'].textContent = text(data.timezone); elements['postcode-value'].textContent = data.postcode ? t('postcodePrefix', { value: data.postcode }) : t('noPostcode');
